# =============================================================================
# NOVA Bot — Complete Single-File Source (v3.9.2)
# API: https://shopify-api-production-f49a.up.railway.app/check
# Site Filter: Live + $0.50-$5.00 products only
# Status Emojis: 🥇 🥈 🥉 🥔 🧨
# Welcome: Original layout preserved
# Changelog:
#   v3.9.2:
#     - Worker counts raised: MSP=50, SP=50, semaphores raised
#     - Faster mass-check throughput with a proper proxy pool
#   v3.9.1:
#     - Fixed SyntaxError in cmd_cmds (return if not await ...)
#     - test_proxy() verifies real Shopify reachability (not just ipify)
#     - test_site_with_price() labels proxy / 407 / timeout failures as 'proxy_error'
#     - /addsites and /site count and abort on proxy errors
#     - Added /resetsites (wipe + test)
#     - Site test timeout raised to 45s for slow proxies
# =============================================================================
...
async def test_proxy(proxy: str):
    """
    A proxy is only 'alive' if it can actually reach a Shopify storefront.
    Pinging ipify alone is not enough — many proxies reach the internet but
    get 407'd / blocked by Shopify's CDN.
    """
...
async def test_site_with_price(site: str, proxy: str = None) -> dict:
    """Test site. Returns:
        ...
        status='proxy_error'  → proxy failed (407 / timeout / DNS)
    """
    ...
    try:
        ...
        # ---- Proxy failures (do not blame the site) ----
...
async def _run_site_tests(event, uid, to_test, status_msg, initial_sites_count, existing=None, wipe=False):
    """Shared routine used by /addsites, /site and /resetsites."""
    ...
    try:
        ...
        # Abort with clear signal if the pool is broken
        if proxy_err > 0 and len(alive) == 0 and over == 0 and not wipe:
            return await status_msg.edit(pe(
                ...
                f"💡 {bs('Your proxies are rejecting requests (HTTP 407 / timeout)')}\n"
                f"💡 {bs('Replace them with working proxies and retry')}"
            ), parse_mode='html', link_preview=False)
...
async def main():
    ...
    try:
        ...
        pass
    log_system("BOOT", "Starting NOVA bot v3.9.2...")
    if not BOT_TOKEN:
