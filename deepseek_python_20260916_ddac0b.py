# =============================================================================
# NOVA Bot — Complete Single-File Source (v5.1.2)
# =============================================================================
# v5.1.2: Engine-aligned proxy check (curl_cffi health via checkout_engine)
#         + SITE_TEST debug logging in _run_site_tests
#         + Lower SITE_PER_USER_WORKERS (20 → 4) to avoid Shopify 429
#         + Reclassify HTTP 429 as retryable proxy_error instead of dead
# v5.1.1: /sh and /msh commands now live ONLY inside the Gates button menu.
...
# ── Engine-aligned proxy health check (optional import) ──
try:
...
SITE_PER_USER_WORKERS  = 4      # v5.1.2: lowered from 20 to avoid Shopify 429
...
async def test_proxy(proxy: str):
    ...
    v5.1.2: also confirms the engine's curl_cffi TLS layer can reach Shopify.
    """
    ...
    try:
        async with session.get(
            ...
            if r.status in (200, 301, 302, 401, 403, 404):
                # ── v5.1.2: engine-aligned TLS check via curl_cffi ──
...
async def test_site_with_price(site: str, proxy: str = None) -> dict:
    ...
    if any(k in low for k in ('timeout', 'curl: (28)')):
        return {'site': site, 'status': 'proxy_error', 'price': 0.0,
                'response': 'Timeout'}
    # ── v5.1.2: rate-limit detection — retryable ──
...
async def cmd_api(event):
    ...
    await styled_reply(event, pe(
        ...
        f"✅ {bs('Engine')}: <code>checkout_engine v2.4.1</code>\n"
...
async def cmd_diag(event):
    ...
    lines.append(f"📦 Version: <code>v5.1.2</code>")
...
async def main():
    ...
    log_system("BOOT", "Starting NOVA bot v5.1.2 (API-LESS)...")
