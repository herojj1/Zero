...
#   - Extra fallback regexes for PollForReceipt (survives JS mutations)
...
def run_checkout_public(shop_url: str, card: str, proxy_url: str = "", low: bool = True):
    ...
    Returns a dict with:
        ...
        retryable    : bool
        currency     : e.g. 'USD' (v2.4.0+)
        receipt_url  : order URL when available (v2.4.0+)
    """
    try:
...
if __name__ == "__main__":
    ...
    result = run_checkout_public(
        shop,                       # positional 1: shop_url
        f"{card_num}|{exp_m}|{exp_y}|{cvv_code}",   # positional 2: card
        proxy,                      # positional 3: proxy
        True                        # positional 4: low
    )
