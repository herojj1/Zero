# =============================================================================
# NOVA Bot — Complete Single-File Source (v5.2.2)
# =============================================================================
...
async def cmd_api(event):
    ...
    await styled_reply(event, pe(
        ...
        f"✅ {bs('Engine')}: <code>checkout_engine v2.5.2</code>\n"
...
# ====================== /chancheck ======================
@client.on(events.NewMessage(pattern=r'^[/.]chancheck$'))
async def cmd_chancheck(event):
    if event.sender_id not in ADMIN_ID:
        return
    await styled_reply(event, pe(
        f"🔎 <b>{bs('Channel Check')}</b>\n{SEP}\n"
        f"⏳ {bs('Testing all channels...')}"))

    lines = [f"🔎 <b>{bs('Channel Diagnostic')}</b>", SEP]
    me = await client_instance.get_me()
    lines.append(f"🤖 Bot: @{me.username} ({me.id})")
    lines.append(SEP)

    targets = [
        ("HIT_CHANNEL_ID", HIT_CHANNEL_ID),
        ("CHARGED_ONLY_CHANNEL_ID", CHARGED_ONLY_CHANNEL_ID),
        ("GROUP_CHAT_ID", GROUP_CHAT_ID),
        ("REDEEM_LOG_CHANNEL_ID", REDEEM_LOG_CHANNEL_ID),
    ...
    for name, cid in targets:
        ...
        try:
            ent = await client_instance.get_entity(cid)
            title = getattr(ent, 'title', None) or getattr(ent, 'username', '—')
            lines.append(f"  📛 <code>{title}</code>")
        except Exception as e:
            lines.append(f"  📛 ❌ <code>{type(e).__name__}</code>")
            continue
        try:
            test_msg = await send_entities(cid, pe(
                f"🧪 <b>{bs('Channel Test')}</b>\n"
                f"⏰ <code>{datetime.now().strftime('%H:%M:%S')}</code>\n"
                f"✅ {bs('If you see this, posting works')}"))
            if test_msg:
                lines.append(f"  📤 Test post: ✅")
                try:
                    await asyncio.sleep(2)
                    await client_instance.delete_messages(cid, test_msg.id)
            ...
            lines.append(f"  📤 ❌ <code>{type(e).__name__}: {str(e)[:60]}</code>")
    await styled_reply(event, pe("\n".join(lines)))
...
{body}""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="ref_menu",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
    except Exception:
        pass
...
💡 {bs('Run')} <code>• /sh</code> {bs('daily to keep it alive')}""")
    try:
        await event.edit(text, buttons=[
            [pbtn(bs("👤 Full Stats"), data="me_menu", style="primary", icon="👤")],
            [pbtn(bs("🔙 Tools"), data="tools_menu", style="danger", icon="🔙")],
        ], parse_mode='html', link_preview=False)
    except Exception:
        pass
...
💡 {bs('Use')} <code>• /sh</code> {bs('or')} <code>• /msh</code>""")
    ...
    try:
        await event.edit(text, buttons=[
            [pbtn(bs("🎁 Top Refs"), data="ref_leaderboard", style="primary", icon="🎁")],
            [pbtn(bs("🔙 Tools"), data="tools_menu", style="danger", icon="🔙")],
        ], parse_mode='html', link_preview=False)
    except Exception:
        pass
...
└─ <code>• /gen 515462 10</code>""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="lookup_menu",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
    except Exception:
        pass
...
💡 <code>• /fake US</code>""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="sub_fake",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
    except Exception:
        pass
...
└─ <code>NOVA_split_1.txt</code> ...""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="file_menu",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
    except Exception:
        pass
...
┗ 📎 {bs('Any media')}""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="cmd_tools",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
    except Exception:
        pass
...
💡 {bs('Free users auto-use shared pool')}""")
        kb = [[pbtn(bs("🎁 Referrals"), data="ref_menu", style="success", icon="🎁")],
              [pbtn(bs("🔙 Back"), data="main_menu", style="danger", icon="🔙")]]
        try:
            await event.edit(text, buttons=kb, parse_mode='html', link_preview=False)
        except Exception:
...
┗ <code>US:ip:port:user:pass</code>
{SEP}
💡 {bs('Country tag → BIN matching')}""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="proxy_menu",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
    except Exception:
        pass
...
{SEP}
📥 <b>{bs('Add (keep existing)')}</b>
┗ <code>• /addsites</code> <i>(reply .txt)</i>
{SEP}
🔄 <b>{bs('Reset (wipe + test)')}</b>
...
┣ <code>• /setminprice 0.01</code>
┗ <code>• /getminprice</code>
{SEP}
📁 <b>{bs('Export')}</b>
┗ <code>• /getsites</code>""")
    kb = [
        [pbtn(bs("📥 Add"), data="sub_addsites", style="success", icon="📥"),
         pbtn(bs("🔄 Reset"), data="sub_resetsites", style="danger", icon="🔄")],
...
async def cb_sub_addsites(event):
    await event.answer()
    text = pe(f"""📥 <b>{bs('Add Sites')}</b> (keeps existing)
{SEP}
<b>{bs('Usage')}</b>
└─ Reply .txt → <code>• /addsites</code>
{SEP}
<b>{bs('Behavior')}</b>
┣ ✅ {bs('Appends new sites')}
┣ ✅ {bs('Keeps existing list')}
┣ ❌ {bs('Skips duplicates')}
┗ 🎯 {bs('Range')}: <code>${get_min_price():.2f}–${get_threshold():.2f}</code>
{SEP}
💡 {bs('Use')} <code>• /resetsites</code> {bs('to wipe first')}""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="cmd_sites",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
    except Exception:
        pass
...
@client.on(events.CallbackQuery(data=b"sub_resetsites"))
async def cb_sub_resetsites(event):
    await event.answer()
    text = pe(f"""🔄 <b>{bs('Reset Sites')}</b> (wipes list)
{SEP}
<b>{bs('Usage')}</b>
└─ Reply .txt → <code>• /resetsites</code>
{SEP}
<b>{bs('Behavior')}</b>
┣ ⚠️ {bs('WIPES current sites.txt')}
┣ ✅ {bs('Tests new list')}
┗ 🎯 {bs('Range')}: <code>${get_min_price():.2f}–${get_threshold():.2f}</code>
{SEP}
💡 {bs('Use only if you want a fresh start')}""")
    try:
        await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="cmd_sites",
                                              style="danger", icon="🔙")]],
                         parse_mode='html', link_preview=False)
...
💎 <b>{bs('Reward')}</b>: {REFERRAL_MILESTONE_EVERY} refs → <code>+{REFERRAL_MILESTONE_HOURS}h +{REFERRAL_MILESTONE_CC_LIMIT} CC</code>""")
    await event.edit(text, buttons=[[pbtn(bs("🔙 Back"), data="back_cmds",
                                          style="danger", icon="🔙")]],
                     parse_mode='html', link_preview=False)
...
{SEP}
💡 {bs('Premium only')}
💡 {bs('Tag with')} <code>US:</code> <code>DE:</code> {bs('for BIN matching')}""")
    await event.edit(text, buttons=[
...
async def cmd_reftop(event):
    ...
    for i, (uid_s, cnt, mst, hrs) in enumerate(rows, 1):
        try:
            ...
            name = uid_s
        lines.append(f"{i}. <code>{uid_s}</code> <b>{name}</b> — {cnt} · {mst}🏅 · {hrs}h")
    await styled_reply(event, pe(f"🏆 <b>{bs('Top Referrers')}</b>\n{SEP}\n" + "\n".join(lines)))
...
async def cmd_diag(event):
    ...
    lines.append(f"📦 Version: <code>v5.2.2</code>")
...
┣ <code>• /sitestats</code> · <code>• /resetstats</code>
{SEP}
📊 <b>{bs('Bot')}</b>
...
┣ <code>• /chancheck</code>
┗ <code>• /diag</code> · <code>• /fjtest</code> · <code>• /getid</code> · <code>• /tail</code>
{SEP}
🎬 <b>{bs('Videos')}</b>
...
async def main():
    ...
    log_system("BOOT", "Starting NOVA bot v5.2.2 (API-LESS)...")
