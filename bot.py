# =============================================================================
# NOVA Bot — Complete Single-File Source (v3.9.0)
...
async def cmd_start(event):
    ...
    # ── ORIGINAL welcome text (unchanged layout) ──
    text = pe(f"""{SEP}
    ✨ {bs('Welcome to NOVA')} ✨
{SEP}
👤 {bs('User')}: @{username}
...
async def cmd_cmds(event):
    if await check_maintenance(event):
        return    if not await force_join_check(event):
        return
    kb = [
...
async def main():
    ...
    try:
        ...
        pass
    log_system("BOOT", "Starting NOVA bot v3.9.0...")
    if not BOT_TOKEN:
