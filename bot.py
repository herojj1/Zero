# =============================================================================
# NOVA Bot — Complete Single-File Source (v3.2)
# API: https://web-production-43fc5.up.railway.app/check
# Fixes: URL encoding, real-charge verification, error handling, Razorpay disabled
# =============================================================================
...
def is_site_error(t):
    ...
    dead_markers = [
        ...
        "this store is unavailable", "store closed", "shop closed",
...
def build_shopify_url(site: str, card: str, proxy: str = None) -> str:
    """Build the API URL with proper URL-encoding for all params."""
...
def classify_shopify(raw: dict, card: str, site: str = None, proxy=None):
    """
    Classify the API response.
      - status_code starting with PROXY_/CARD_/URL_/etc. → Error (API-side issue)
      - Response == ERROR / SITE ERROR → Error (couldn't complete check)
      - Response == CHARGED → Charged
      - Response == APPROVED → Approved
      - Response == DECLINED → Dead
      - Response == 3DS / ActionRequired → 3DS
    """
    response = str(raw.get('Response', '')).strip()
    resp_up = response.upper()
    resp_low = response.lower()
    status_code = str(raw.get('status_code', '')).upper()
    error_field = str(raw.get('error', '')).strip()
    ...
    charged_flag = str(raw.get('Charged', '')).lower()

    # ── API-side errors (not card declines) ──
    api_error_codes = {
        'PROXY_REQUIRED', 'PROXY_INVALID', 'CARD_INVALID', 'CARD_REQUIRED',
        'CARD_EXPIRED', 'URL_INVALID', 'URL_REQUIRED',
    }
    ...
    # ── Explicit CHARGED (from fixed API) ──
    if resp_up == 'CHARGED' or charged_flag == 'true':
        return {'status': 'Charged', 'message': response or 'Charged',
                'card': card, 'site': site, 'gateway': gateway,
                'price': price_disp, 'price_value': price_val, 'proxy': proxy}

    # ── Explicit APPROVED ──
    if resp_up in ('APPROVED', 'APPROVED_PAYMENT_PROCESSING') or \
       status_code in ('PAYMENT_PROCESSING', '3DS_AUTHENTICATION', 'INSUFFICIENT_FUNDS'):
        # Note: if status_code is 3DS, mark as 3DS below
        if status_code == '3DS_AUTHENTICATION' or '3ds' in resp_low:
            return {'status': '3DS', 'message': response or status_code,
                    'card': card, 'site': site, 'gateway': gateway,
                    'price': price_disp, 'price_value': price_val, 'proxy': proxy}
    ...
    # ── API ERROR — treat as retryable, NOT a card decline ──
    if resp_up in ('ERROR', 'SITE ERROR') or not response:
        return {'status': 'Error',
                'message': response or error_field or 'API error',
                'card': card, 'site': site, 'gateway': gateway,
                'price': price_disp, 'price_value': price_val, 'proxy': proxy}

    # ── Legacy fallbacks for non-fixed APIs ──
        ...
        async with session.get(url) as resp:
            if resp.status == 429:
                return {'status': 'Error', 'message': 'API rate limited (429)',
                        ...
                        'price': '-', 'price_value': 0.0, 'proxy': proxy}, proxy
...
async def check_card_with_retry(card: str, sites: list, proxies: list,
    ...
    for attempt in range(max_retries):
        ...
        # Success (non-error) — return immediately
        if status in ('Charged', 'Approved', '3DS', 'Dead'):
            rotator.report_site_ok(site)
            if proxy:
                rotator.report_proxy_ok(proxy)
            return result, last_proxy

        # status == 'Error' — retry with different site/proxy
...
def _status_header(status: str):
    ...
    if s == 'ERROR':    return f"{PE} <b>{bs('ERROR')}</b>"
...
async def cmd_sh(event):
    ...
    try:
        ...
        if status in ("Charged", "Approved", "3DS"):
            ...
            # Includes Dead AND Error — show result inline
            try:
...
async def cmd_rz(event):
    ...
    # ... (Razorpay path only runs if RAZORPAY_ENABLED = True)
    uid = event.sender_id
    if await check_maintenance(event):
        return
    if not await force_join_check(event):
...
⚠️ {bs('Errors')}: {len(results.get('errors', []))}
...
async def start_mass_rz(user_id: int, cards: list, event, status_msg):
    if not RAZORPAY_ENABLED:
        return
    # ... (Razorpay mass code only runs if RAZORPAY_ENABLED = True)
    proxies = load_user_proxies(user_id)
    if not proxies:
        await styled_edit(status_msg, pe(f"💎 <b>{bs('No proxies')}</b> — /addproxy"))
        return
...
async def test_site_with_price(site: str, proxy: str = None) -> dict:
    """Returns status: alive / dead / unknown."""
    test_card = "5154623245618097|03|2032|156"
    if not site:
        return {'site': site, 'status': 'dead', 'price': 0.0, 'response': 'Empty'}
    if not _proxy_str_for_api(proxy):
        return {'site': site, 'status': 'unknown', 'price': 0.0, 'response': 'No proxy'}
    ...
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=45, connect=20)) as resp:
            if resp.status in (429, 500, 502, 503, 504):
                return {'site': site, 'status': 'unknown', 'price': 0.0,
        ...
        sc = str(raw.get('status_code', '')).upper()
        err = str(raw.get('error', '')).lower()
        response = str(raw.get('Response', '')).strip()

        if sc in ('PROXY_REQUIRED', 'PROXY_INVALID'):
            return {'site': site, 'status': 'unknown', 'price': 0.0,
                    'response': sc}
        if sc in ('URL_INVALID', 'URL_REQUIRED'):
            return {'site': site, 'status': 'dead', 'price': 0.0,
                    'response': sc}
        if response.upper() in ('ERROR', 'SITE ERROR', ''):
            return {'site': site, 'status': 'unknown', 'price': 0.0,
        ...
        return {'site': site, 'status': 'unknown', 'price': 0.0, 'response': 'Timeout'}
    except Exception as e:
        return {'site': site, 'status': 'unknown', 'price': 0.0,
                'response': str(e)[:50]}
...
async def cmd_addsites(event):
    ...
    alive, dead, over, checked, unknown = [], 0, 0, 0, 0

    async def _check_one(site):
        nonlocal checked, dead, over, unknown
        async with sem:
        ...
        if res['status'] == 'alive':
            if res['price'] <= threshold:
                ...
                over += 1
                alive.append(site)   # keep even if over threshold
        elif res['status'] == 'unknown':
            unknown += 1
            alive.append(site)       # keep unknowns — may work later
        else:
            dead += 1
    ...
    try:
        for i in range(0, len(to_test), SITE_PER_USER_WORKERS):
            ...
            try:
                await status_msg.edit(pe(
                    ...
                    f"⚠️ <code>{unknown}</code> | 💰 <code>{over}</code>"
                ), parse_mode='html', link_preview=False)
            except Exception:
                pass
        if alive:
        ...
        await status_msg.edit(pe(
            ...
            f"⚠️ {bs('Unknown (kept)')}: <code>{unknown}</code>\n"
            f"💰 {bs('Over threshold')}: <code>{over}</code>\n"
            f"🔁 {bs('Dupes')}: <code>{already}</code>\n"
...
FAKE_DATA = {
    ...
    'GB': {'name':'United Kingdom','phone_code':'+44','len':10,
        ...
        'streets':['High St','Church St','Queen St','King St','Park Ave','Victoria Rd'],
        'first':['Oliver','George','Harry','Jack','Jacob','Charlie','Thomas','Oscar','William','Henry'],
    ...
    'DE': {'name':'Germany','phone_code':'+49','len':11,
        ...
        'first':['Lukas','Maximilian','Felix','Jonas','Leon','Paul','Elias','Noah'],
        'last':['Müller','Schmidt','Schneider','Fischer','Weber','Meyer','Wagner','Becker']},
    'FR': {'name':'France','phone_code':'+33','len':9,
        'cities':[('Paris','Île-de-France','75001'),('Marseille','PACA','13001'),('Lyon','ARA','69001')],
    ...
    'IT': {'name':'Italy','phone_code':'+39','len':10,
        ...
        'streets':['Via Roma','Via Garibaldi','Via Dante','Corso Italia'],
    ...
    'SE': {'name':'Sweden','phone_code':'+46','len':9,
        'cities':[('Stockholm','AB','11122'),('Göteborg','VG','41101'),('Malmö','M','21121')],
        'streets':['Storgatan','Kungsgatan','Drottninggatan'],
        'first':['William','Lucas','Liam','Elias','Oscar','Hugo'],
        'last':['Andersson','Johansson','Karlsson','Nilsson','Eriksson','Larsson']},
    'NO': {'name':'Norway','phone_code':'+47','len':8,
        'cities':[('Oslo','Oslo','0150'),('Bergen','Vestland','5003')],
        'streets':['Storgata','Karl Johans gate'],
        'first':['Jakob','Emil','Noah','Oliver','William'],
    ...
    'DK': {'name':'Denmark','phone_code':'+45','len':8,
        ...
        'streets':['Hovedgaden','Nørregade'],
        'first':['William','Noah','Oscar','Lucas','Victor'],
        'last':['Nielsen','Jensen','Hansen','Pedersen','Andersen']},
    'IE': {'name':'Ireland','phone_code':'+353','len':9,
        'cities':[('Dublin','Leinster','D01'),('Cork','Munster','T12')],
        'streets':['Main St',"O'Connell St",'Grafton St'],
        'first':['Jack','James','Daniel','Conor','Sean','Adam'],
        'last':["Murphy","Kelly","O'Brien","Walsh","Smith"]},
    ...
    'PK': {'name':'Pakistan','phone_code':'+92','len':10,
        ...
        'last':['Khan','Ahmed','Malik','Sheikh','Chaudhry']},
    'CN': {'name':'China','phone_code':'+86','len':11,
        'cities':[('Beijing','Beijing','100000'),('Shanghai','Shanghai','200000')],
        'streets':["Chang'an Ave",'Nanjing Road','Huaihai Road'],
        'first':['Wei','Jun','Ming','Yan','Lei','Hao'],
        'last':['Wang','Li','Zhang','Liu','Chen','Yang']},
    'JP': {'name':'Japan','phone_code':'+81','len':10,
        'cities':[('Tokyo','Tokyo','100-0001'),('Osaka','Osaka','530-0001')],
        'streets':['Chuo-dori','Ginza','Shibuya'],
        'first':['Haruto','Yuto','Sota','Yuki','Hayato','Ren'],
        'last':['Sato','Suzuki','Takahashi','Tanaka','Watanabe']},
    'KR': {'name':'South Korea','phone_code':'+82','len':10,
        'cities':[('Seoul','Seoul','03000'),('Busan','Busan','48000')],
        'streets':['Gangnam-daero','Sejong-daero'],
        'first':['Min-jun','Ji-ho','Seo-jun','Do-yun','Ha-jun','Eun-woo'],
    ...
    'TH': {'name':'Thailand','phone_code':'+66','len':9,
        ...
        'streets':['Sukhumvit Rd','Silom Rd'],
        'first':['Somchai','Somsak','Sombat','Prasert','Anan'],
        'last':['Srisuk','Wongchai','Saelim','Chaiyasit']},
    'VN': {'name':'Vietnam','phone_code':'+84','len':9,
        'cities':[('Hà Nội','Hà Nội','100000'),('Hồ Chí Minh','HCMC','700000')],
        'streets':['Đường Lê Lợi','Đường Nguyễn Huệ'],
        'first':['Minh','Hùng','Tuấn','Nam','Long'],
        'last':['Nguyễn','Trần','Lê','Phạm','Hoàng']},
    'MY': {'name':'Malaysia','phone_code':'+60','len':9,
        'cities':[('Kuala Lumpur','KL','50000'),('Penang','Penang','10000')],
        'streets':['Jalan Ampang','Jalan Bukit Bintang'],
        'first':['Ahmad','Muhammad','Aiman','Hakim','Irfan','Danish'],
        'last':['Abdullah','Ibrahim','Hassan','Ismail','Yusof','Rahman']},
    'SG': {'name':'Singapore','phone_code':'+65','len':8,
        'cities':[('Singapore','Central','018956'),('Jurong','West','609390')],
        'streets':['Orchard Road','Marina Bay','Bugis Street'],
        'first':['Wei Ming','Jun Wei','Jia Hui','Kai Wen','Zhi Hao'],
        'last':['Tan','Lim','Lee','Ng','Ong','Wong']},
    'ID': {'name':'Indonesia','phone_code':'+62','len':10,
        'cities':[('Jakarta','DKI','10110'),('Surabaya','JT','60111')],
        'streets':['Jl. Sudirman','Jl. Thamrin','Jl. Gatot Subroto'],
        'first':['Budi','Andi','Agus','Dedi','Rudi','Hendra'],
        'last':['Santoso','Wijaya','Saputra','Susanto','Kurniawan']},
    'PH': {'name':'Philippines','phone_code':'+63','len':10,
        'cities':[('Manila','NCR','1000'),('Quezon City','NCR','1100')],
        'streets':['Ayala Ave','EDSA','Roxas Blvd'],
        'first':['Juan','Jose','Antonio','Pedro','Mark','John'],
        'last':['Santos','Reyes','Cruz','Bautista','Ocampo']},
    'AE': {'name':'United Arab Emirates','phone_code':'+971','len':9,
        'cities':[('Dubai','Dubai','00000'),('Abu Dhabi','Abu Dhabi','00001')],
        'streets':['Sheikh Zayed Road','Jumeirah Beach Road'],
        'first':['Mohammed','Ahmed','Ali','Omar','Khalid','Saeed'],
        'last':['Al Maktoum','Al Nahyan','Al Falasi','Al Suwaidi']},
    ...
    'RU': {'name':'Russia','phone_code':'+7','len':10,
        ...
        'streets':['ul. Lenina','ul. Pushkina','ul. Gagarina'],
        'first':['Aleksandr','Sergey','Dmitriy','Andrey','Aleksey','Ivan'],
    ...
    'BR': {'name':'Brazil','phone_code':'+55','len':11,
        ...
        'streets':['Avenida Paulista','Rua Augusta','Avenida Brasil'],
    ...
    'NG': {'name':'Nigeria','phone_code':'+234','len':10,
        'cities':[('Lagos','Lagos','100001'),('Abuja','FCT','900001')],
        'streets':['Broad Street','Marina','Allen Avenue'],
        'first':['Chinedu','Emeka','Adebayo','Oluwaseun','Ifeanyi','Tunde'],
        'last':['Okafor','Adeyemi','Okonkwo','Balogun','Nwosu']},
    'ZA': {'name':'South Africa','phone_code':'+27','len':9,
        'cities':[('Johannesburg','Gauteng','2000'),('Cape Town','WC','8000')],
        'streets':['Main Rd','Church St','Long St'],
        'first':['Johan','Pieter','Jacobus','Thabo','Sipho','David'],
        'last':['Van der Merwe','Botha','Pretorius','Nkosi','Dlamini']},
    'EG': {'name':'Egypt','phone_code':'+20','len':10,
        'cities':[('Cairo','Cairo','11511'),('Alexandria','Alexandria','21511')],
        'streets':['Tahrir Square','Corniche El Nil','Ramses Street'],
        'first':['Ahmed','Mohamed','Mahmoud','Omar','Youssef','Ali'],
        'last':['Hassan','Ibrahim','Ahmed','El-Sayed','Abdel-Rahman']},
}
