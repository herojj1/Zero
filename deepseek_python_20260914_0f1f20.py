# =============================================================================
# NOVA Bot — Complete Single-File Source (v4.7.0 — API-LESS)
# =============================================================================
# Changelog v4.7.0 (from v4.6.0):
#   - API-LESS: bot talks DIRECTLY to Shopify. No Railway/external API server.
#   - Uses checkout_engine.py (curl_cffi, TLS fingerprint spoofing)
#   - check_card_shopify() runs engine in thread pool
#   - test_site_with_price() runs engine directly
#   - /api command shows local engine health
#   - SHOPIFY_API_URL is now a stub ("direct") — kept only for compat
#   - Everything else from v4.6.0 unchanged
#
# Changelog v4.6.0 (from v4.3.0):
#   - FIX: strict force-join (both chats, fails closed on error)
#   - FIX: site testing accepts DECLINED/CHARGED/APPROVED as reachable
#   - FIX: /sk (Stripe key check) — proper headers, all key types, full info
#   - NEW: /version, /getid, /fjtest, /diag diagnostic commands
#   - Force-join IDs: Group=-1003902938287, Channel=-1004381920430
#   - /fb locked to Premium + Admin
# =============================================================================
...
try:
    from checkout_engine import (
        ...
        parse_card_entry   as engine_parse_card,
        CheckStatus,
    )
    ENGINE_AVAILABLE = True
except ImportError as _engine_err:
    ENGINE_AVAILABLE = False
    _ENGINE_IMPORT_ERROR = str(_engine_err)

# Thread pool for running the blocking engine without freezing the bot
_ENGINE_POOL = concurrent.futures.ThreadPoolExecutor(
    max_workers=50,
    thread_name_prefix="shopify",
)
...
# API-LESS — no external API server. Kept as stub for display compatibility.
SHOPIFY_API_URL  = "direct"
...
def build_status_text():
    ...
    try:
        ...
        engine_line = "🟢 API-LESS" if ENGINE_AVAILABLE else "🔴 engine missing"
...
# ====================== SHOPIFY ENGINE (API-LESS) ======================
_ENGINE_STATUS_MAP = {
    CheckStatus.CHARGED:  "Charged",
    CheckStatus.APPROVED: "Approved",
    CheckStatus.DECLINED: "Dead",
    CheckStatus.ERROR:    "Error",
} if ENGINE_AVAILABLE else {}


def _proxy_for_engine(proxy: str) -> str:
    if not proxy:
        return ""
    try:
        return engine_normalize_proxy(proxy)
    except Exception:
        return ""
...
async def check_card_shopify(card: str, site: str, proxy: str = None, http_session=None):
    ...
    if not ENGINE_AVAILABLE:
        return {'status': 'Error',
                'message': f'checkout_engine not loaded: {_ENGINE_IMPORT_ERROR}',
                'card': card, 'site': site, 'gateway': 'Shopify', 'price': '-',
                'price_value': 0.0, 'proxy': proxy, 'retryable': False}, None

    shop_url = site if site.startswith("http") else f"https://{site}"
    proxy_url = _proxy_for_engine(proxy)
    loop = asyncio.get_event_loop()

    def _run():
        return run_checkout_for_card(shop_url, card, proxy_url, low=True)
    ...
    try:
        ...
        return {'status': 'Error', 'message': 'Engine timeout (180s)',
                'card': card, 'site': site, 'gateway': 'Shopify',
                'price': '-', 'price_value': 0.0, 'proxy': proxy,
                'retryable': True}, proxy
    except Exception as e:
        return {'status': 'Error', 'message': f'{type(e).__name__}: {e}'[:100],
                'card': card, 'site': site, 'gateway': 'Shopify',
                'price': '-', 'price_value': 0.0, 'proxy': proxy,
                'retryable': True}, proxy

    status_str = _ENGINE_STATUS_MAP.get(result.status, "Error")

    try:
        price_val = float(result.amount) if result.amount else 0.0
    except (ValueError, TypeError):
        price_val = 0.0
    price_disp = f"${result.amount}" if result.amount else "-"

    if result.status_code == "3DS_AUTHENTICATION":
        status_str = "3DS"

    return {
        'status':      status_str,
        'message':     result.status_code or (str(result.error) if result.error else ""),
        'card':        card,
        'site':        site,
        'gateway':     "Shopify",
        'price':       price_disp,
        ...
        'receipt_url': result.receipt_url or "",
    }, proxy
...
async def check_card_with_retry(card: str, sites: list, proxies: list,
    ...
    for _ in range(max_retries):
        ...
        api_retry   = result.get('retryable', False)

        if status in ('Charged', 'Approved', '3DS'):
            rotator.report_site_ok(site)
            if proxy: rotator.report_proxy_ok(proxy)
            return result, last_proxy

        if status == 'Dead' and not api_retry:
            rotator.report_site_ok(site)
            if proxy: rotator.report_proxy_ok(proxy)
            return result, last_proxy

        rotator.report_site_fail(site)
        if proxy and is_proxy_error(result.get('message', '')):
...
async def cmd_api(event):
    ...
    if not ENGINE_AVAILABLE:
        return await styled_reply(event, pe(
            f"❌ <b>{bs('Engine Missing')}</b>\n{SEP}\n"
            f"📌 Mode: <code>API-LESS</code>\n"
            f"🔴 <code>checkout_engine.py not importable</code>\n"
            f"💡 Error: <code>{_ENGINE_IMPORT_ERROR}</code>"
        ))
    await styled_reply(event, pe(
        f"🔌 <b>{bs('Engine Health')}</b> (API-LESS)\n{SEP}\n"
        f"📌 Mode: <code>direct Shopify checkout</code>\n"
        f"🟢 Status: <code>running</code>\n"
        f"🧵 Workers: <code>{pool._max_workers}</code>\n"
        f"📦 Sites: <code>{len(load_sites())}</code>\n"
        f"🔑 Proxies (you): <code>{len(load_user_proxies(event.sender_id))}</code>\n"
        f"📦 Version: <code>v4.7.0</code>"
    ))
...
async def cmd_version(event):
    ...
    await styled_reply(event, pe(
        ...
        f"📦 Version: <code>v4.7.0</code>\n"
        f"🔌 Engine: <code>API-LESS (direct Shopify)</code>\n"
        f"🟢 Engine loaded: <code>{ENGINE_AVAILABLE}</code>\n"
        f"🔒 Force-join chats: <code>{len(FORCE_JOIN_CHATS)}</code>\n"
...
async def cmd_getid(event):
    ...
    lines = [
        ...
        f"🎯 <b>{bs('Config slot')}</b>: <code>{slot}</code>",
    ]
    if username:
...
async def cmd_fjtest(event):
    ...
    for chat_id, name, link in FORCE_JOIN_CHATS:
        ...
        try:
            ...
            lines.append(f"  🤖 Bot member: ✅")
        except ChatAdminRequiredError:
            lines.append(f"  🤖 Bot member: ❌ <b>BOT NOT ADMIN</b>")
        except ChannelPrivateError:
            lines.append(f"  🤖 Bot member: ❌ <b>CHAT PRIVATE / WRONG ID</b>")
        except Exception as e:
            lines.append(f"  🤖 Bot member: ❌ <code>{type(e).__name__}</code>")
        try:
            await client_instance(GetParticipantRequest(channel=chat_id, participant=test_uid))
            lines.append(f"  👤 You: ✅ member")
        except UserNotParticipantError:
            lines.append(f"  👤 You: ⚠️ not a member")
        except ChatAdminRequiredError:
            lines.append(f"  👤 You: ❌ <b>CANNOT CHECK — BOT NOT ADMIN</b>")
        except Exception as e:
...
async def cmd_diag(event):
    ...
    lines.append(f"📦 Version: <code>v4.7.0</code>")
    lines.append(f"🔌 Engine: <code>API-LESS</code>")
    lines.append(f"🟢 Engine loaded: <code>{ENGINE_AVAILABLE}</code>")
    lines.append(f"🔒 Force-join chats: <code>{len(FORCE_JOIN_CHATS)}</code>")
    for cid, name, link in FORCE_JOIN_CHATS:
    ...
    for chat_id, name, link in FORCE_JOIN_CHATS:
        ...
        try:
            await client_instance(GetParticipantRequest(channel=chat_id, participant=event.sender_id))
            lines.append(f"    👤 Test user: ✅ member")
        except UserNotParticipantError:
            lines.append(f"    👤 Test user: ⚠️ not member")
        except ChatAdminRequiredError:
            lines.append(f"    👤 Test user: ❌ <b>CANNOT CHECK</b>")
        except Exception as e:
            lines.append(f"    👤 Test user: ❌ <code>{type(e).__name__}</code>")
    lines.append(SEP)
    sites = load_sites()
    ...
    if sites and proxies:
        lines.append(f"🧪 <b>Live engine test (API-LESS)</b>")
        test_site = sites[0]
        test_proxy = proxies[0]
        test_card = "4111111111111111|12|2030|123"
        lines.append(f"  Site: <code>{test_site}</code>")
        try:
            result, _ = await check_card_shopify(test_card, test_site, test_proxy, None)
            lines.append(f"  Status: <code>{result.get('status')}</code>")
            lines.append(f"  Code: <code>{result.get('status_code','-')}</code>")
            lines.append(f"  Msg: <code>{(result.get('message') or '')[:150]}</code>")
        except Exception as e:
            lines.append(f"  ❌ <code>{type(e).__name__}: {e}</code>")
    await styled_reply(event, pe("\n".join(lines)))
...
def _stripe_key_kind(key: str) -> str:
    k = (key or "").strip()
    if k.startswith("sk_live_"):  return "Live Secret"
    if k.startswith("sk_test_"):  return "Test Secret"
    if k.startswith("rk_live_"):  return "Live Restricted"
    if k.startswith("rk_test_"):  return "Test Restricted"
    if k.startswith("pk_live_"):  return "Live Publishable"
    if k.startswith("pk_test_"):  return "Test Publishable"
    return "Unknown"
...
async def cmd_sk(event):
    ...
    if len(parts) < 2 or not parts[1].strip():
        return await styled_reply(event, pe(
            f"💎 <b>{bs('Stripe Key Check')}</b>\n{SEP}\n"
            f"💡 <code>/sk sk_live_...</code>\n"
            f"💡 <code>/sk sk_test_...</code>\n"
            f"💡 <code>/sk rk_live_...</code>"
        ))
    key = parts[1].strip().strip('"').strip("'")
    valid_prefixes = ("sk_live_", "sk_test_", "rk_live_", "rk_test_")
    if not key.startswith(valid_prefixes):
        return await styled_reply(event, pe(
            f"❌ <b>{bs('Invalid key format')}</b>\n{SEP}\n"
            f"💡 Must start with one of:\n"
            f"<code>sk_live_</code>, <code>sk_test_</code>,\n"
            f"<code>rk_live_</code>, <code>rk_test_</code>"
        ))
    kind = _stripe_key_kind(key)
    status_msg = await styled_reply(event, pe(
        f"💎 <b>{bs('Checking')}</b> <code>{kind}</code>..."
    ))
    ...
    try:
        headers = {
            ...
            "User-Agent": "Stripe/v1 PythonBindings/5.0.0",
        }
        async with session.get(
        ...
        if http_status != 200:
            ...
            return await styled_edit(status_msg, pe(
                ...
                f"📝 Error: <code>{err_msg or body_snip}</code>"
            ))
        acct_id      = data.get("id", "—")
        business     = data.get("business_name") or data.get("settings", {}).get("dashboard", {}).get("display_name") or "—"
        country      = data.get("country", "—")
        currency     = data.get("default_currency", "—").upper()
        email        = data.get("email") or "—"
        charges_on   = bool(data.get("charges_enabled", False))
        payouts_on   = bool(data.get("payouts_enabled", False))
        details_done = bool(data.get("details_submitted", False))
        caps         = data.get("capabilities", {}) or {}
        card_pay     = caps.get("card_payments", "—")
        live_mode    = bool(data.get("livemode", False))
        masked = key[:10] + "…" + key[-4:] if len(key) > 16 else key[:6] + "…"
        emoji_charges = "🟢" if charges_on else "🔴"
        emoji_payouts = "🟢" if payouts_on else "🔴"
        emoji_live    = "🔥 LIVE" if live_mode else "🧪 TEST"
        await styled_edit(status_msg, pe(
            f"✅ <b>{bs('Stripe Key Valid')}</b>\n{SEP}\n"
            f"🔑 <code>{masked}</code>\n"
            f"🏷️ {bs('Kind')}: <code>{kind}</code> {emoji_live}\n"
            f"🆔 {bs('Account')}: <code>{acct_id}</code>\n"
            f"🏦 {bs('Business')}: <code>{business}</code>\n"
            f"🌍 {bs('Country')}: <code>{country}</code>\n"
            f"💱 {bs('Currency')}: <code>{currency}</code>\n"
            f"📧 {bs('Email')}: <code>{email}</code>\n"
            f"{SEP}\n"
            f"{emoji_charges} {bs('Charges')}: <code>{charges_on}</code>\n"
            f"{emoji_payouts} {bs('Payouts')}: <code>{payouts_on}</code>\n"
            f"📋 {bs('Details Submitted')}: <code>{details_done}</code>\n"
            f"💳 {bs('Card Payments')}: <code>{card_pay}</code>"
        ))
    except asyncio.TimeoutError:
        await styled_edit(status_msg, pe(
            f"❌ <b>{bs('Timeout')}</b>\n{SEP}\n"
            f"🔑 Kind: <code>{kind}</code>\n"
            f"💡 Stripe API didn't respond in 20s"
        ))
    except Exception as e:
        await styled_edit(status_msg, pe(
            f"❌ <b>{bs('Error')}</b>\n{SEP}\n"
...
async def cmd_stats(event):
    ...
    engine_line = f"🟢 API-LESS" if ENGINE_AVAILABLE else f"🔴 engine missing"
    await styled_reply(event, pe(
        f"📊 <b>{bs('Stats')}</b>\n{SEP}\n"
...
async def cmd_fb(event):
    ...
    if not event.reply_to_msg_id:
        return await styled_reply(event, pe(f"💎 {bs('Reply to a media message')}"))
    reply = await event.get_reply_message()
    if not (reply.media or reply.photo or reply.document or reply.video):
        return await styled_reply(event, pe(f"💎 {bs('No media to forward')}"))
    try:
...
<code>/msh</code> ━ {bs('Mass check (reply to .txt file)')}
{SEP}
💡 <i>{bs('API-LESS engine — direct Shopify')}</i>""")
...
{SEP}
<b>🩺 Diagnostics</b>
<code>/version</code> · <code>/getid</code> · <code>/fjtest</code> · <code>/diag</code>
{SEP}
<b>🎬 Videos</b>
...
async def main():
    ...
    try:
        ...
        pass
    log_system("BOOT", "Starting NOVA bot v4.7.0 (API-LESS)...")
    if not ENGINE_AVAILABLE:
        log_system("BOOT", f"⚠️  checkout_engine.py NOT loaded: {_ENGINE_IMPORT_ERROR}", "error")
        log_system("BOOT", "   Make sure checkout_engine.py is in the same folder.", "error")
        log_system("BOOT", "   Install: pip install curl-cffi", "error")
    else:
        log_system("BOOT", "✅ checkout_engine loaded (API-LESS mode).")
    if not BOT_TOKEN:
    ...
    while True:
        try:
            ...
            log_system("BOOT", "✅ NOVA bot online (API-LESS).")
